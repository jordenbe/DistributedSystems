...
public class CarRentalModel {
	...
	public Set<String> getCarTypesNames(String crcName) {
		String query = "SELECT crc.carTypes FROM CarRentalCompany crc WHERE crc.name = :name";
		EntityManager em = emf.createEntityManager();
		try {
			return new HashSet<>(em.createQuery(query).setParameter("name", crcName).getResultList());	
		}
		finally {
			em.close();	
		}
    ...
    public Collection<String> getAllRentalCompanyNames() {
    	String query = "SELECT crc.name FROM CarRentalCompany crc";
    	EntityManager em = emf.createEntityManager();
    	try {
    ...
    private CarRentalCompany getRentalCompanyByName(String name)
    {
    	EntityManager em = emf.createEntityManager();
    	try
    ...
    public Quote createQuote(String company, String renterName, ReservationConstraints constraints) throws ReservationException {
    	...
    	CarRentalCompany crc =  getRentalCompanyByName(company);
    	Quote out = null;
	...
	public void confirmQuote(Quote q) throws ReservationException {
		// TODO: Reservation laten terug geven?

		CarRentalCompany crc = getRentalCompanyByName(q.getRentalCompany());
        crc.confirmQuote(q);
	}
	...
	public List<Reservation> getReservations(String renter) {
		...
		try {
			String query = "SELECT r FROM Reservation r where r.carRenter = :renter";
    ...
    public Collection<CarType> getCarTypesOfCarRentalCompany(String crcName) {
    	...
    	try {
    		String query = "SELECT crc.carTypes FROM CarRentalCompany crc WHERE carRentalCompany.name = :crcName";
    		return em.createQuery(query,Map.class).setParameter("crcName", crcName).getSingleResult().values();
    	}finally {
    		em.close();
    	}
    }
	...
	private List<Car> getCarsByCarType(String crcName, CarType carType) {				
		...
		try {
			String query = "SELECT r.cars FROM CarRentalCompany r WHERE r.name = :crcName";
			Set<Car> cars = em.createQuery(query,Set.class).setParameter("crcName", crcName).getSingleResult();
			return cars.stream().filter(c -> c.getType().equals(carType)).collect(Collectors.toList());
		}
		finally { em.close();}
	}
